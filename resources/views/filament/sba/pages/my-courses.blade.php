<x-filament-panels::page>
    ...
    <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        @foreach ($rows as $row)
            ...
            @endphp
            {{-- (** executed: same nested-anchor fix as the admin twin. **) --}}
            <div class="group flex flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition hover:border-primary-400 hover:shadow-md dark:border-white/10 dark:bg-gray-900">
                <a href="{{ route('filament.sba.courses.show', $course->slug) }}" class="flex flex-col">
                ...
                @if ($complete)
                    <a href="{{ route('filament.sba.certificates.print', $course->slug) }}" target="_blank"
